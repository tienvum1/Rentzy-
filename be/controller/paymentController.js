...
const createWalletDepositPayment = async (req, res) => {
  try {
    ...
    if (wallet.balance < amount) {
      return res.status(400).json({
        success: false,
        message: `Số dư ví không đủ. Hiện tại: ${wallet.balance.toLocaleString(
          "vi-VN"
        )} VND, Cần: ${amount.toLocaleString("vi-VN")} VND`,
      });
    }
    ...
    console.log(
      `Wallet upfront payment completed: ${amount} VND for booking ${orderCode}`
    );
...
const createWalletRentalPayment = async (req, res) => {
  try {
    ...
    console.log(
      `Wallet rental payment completed: ${amount} VND for booking ${bookingId}`
    );
